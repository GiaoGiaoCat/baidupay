...
class Baidupay::Sign::RSATest < Minitest::Test
  def setup
  ...
  def test_sign
    assert_equal @sign, Baidupay::Sign::RSA.sign(TEST_RSA_PRIVATE_KEY, @string)
  end

  def test_verify
    assert Baidupay::Sign::RSA.verify?(TEST_RSA_PUBLIC_KEY, @string, @sign)
  end

  def test_verify_fail_when_sign_not_true
    assert !Baidupay::Sign::RSA.verify?(TEST_RSA_PUBLIC_KEY, "danger#{@string}", @sign)
  end
end
